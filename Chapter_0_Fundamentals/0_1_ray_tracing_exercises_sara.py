...
def intersect_ray_1d(ray: Float[Tensor, "points dims"], segment: Float[Tensor, "points dims"]) -> bool:
    ...
    # get points
    O, D = ray # which is [[Ox, Oy], [Dx, Dy]]
    L1, L2 = segment # which is [[L1x, L1y],[L2x, L2y]]
...
def intersect_rays_1d(
    ...
    # count number of rays and segments
    num_rays = rays.shape[0]
    num_segments = segments.shape[0]

    # reshape rays and segments tensors using einops so that we can perform a batch computation
    rays = einops.repeat(rays, "rays points dim -> rays new_segments points dim", new_segments = num_segments)
    segments = einops.repeat(segments, "segments points dim -> new_rays segments points dim", new_rays = num_rays)
    ...
    # generate matrix and convert singular matrices into identity matrix to avoid errors later on
    matrix = t.stack([D, L1-L2], dim = -1)
    det = t.linalg.det(matrix)
...
def make_rays_2d(num_pixels_y: int, num_pixels_z: int, y_limit: float, z_limit: float) -> Float[Tensor, "nrays 2 3"]:
    ...
    """
    num_rays = num_pixels_y * num_pixels_z # compute total number of rays
    y_coord = t.linspace(-y_limit, y_limit, num_pixels_y) # generate coordinates in y axis
    z_coord = t.linspace(-z_limit, z_limit, num_pixels_z) # generate coordinates in z axis

    rays_2d = t.zeros((num_rays, 2, 3), dtype = t.float32) # initialize rays tensor

    rays_2d[:, 1, 0] = 1 # set all coordinates of 2nd point in x-axis to 1

    rays_2d[:,1,1] = einops.repeat(y_coord, "y -> (y z)", z = num_pixels_z) # set y coordinates
    rays_2d[:,1,2] = einops.repeat(z_coord, "z -> (y z)", y = num_pixels_y) # set z coordinates
    return rays_2d
...
def raytrace_triangle(
    ...
    # compute number of rays and triangles
    num_rays = rays.shape[0]

    # expand triangle
    triangle = einops.repeat(triangle, "points dim -> new_rays points dim", new_rays = num_rays)

    # extract points of rays and triangles
    O = rays[:, 0, :]
    D = rays[:, 1, :]
    A = triangle[:, 0,:]
    B = triangle[:, 1,:]
    ...
    # find singular matrices and mask
    det = t.linalg.det(matrix)
    is_singular = det.abs() < 1e-8
...
def raytrace_mesh(
    ...
    """
    # get number of rays and triangles
    num_rays = rays.shape[0]
    num_triangles = triangles.shape[0]

    # expand rays and triangles
    rays = einops.repeat(rays, "nrays points dims -> nrays ntriangles points dims", ntriangles = num_triangles)
    triangles = einops.repeat(triangles, "ntriangles points dims -> nrays ntriangles points dims", nrays = num_rays)
    ...
    # get boolean of intersection
    intersects = (u >= 0) & (v >= 0) & (u + v <= 1) & ~is_singular

    # if it doesn't intersect, put an inf value
